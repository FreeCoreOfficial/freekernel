...
int chrysfs_format(block_device_t *dev) {
    ...
    serial("[FS] Creating layout:\n");
    serial("  /root/system/kernel\n");
    serial("  /root/system/drivers\n");
    serial("  /root/users/default/files\n");
    serial("  /root/files\n");
...
int chrysfs_ls(const char *path) {
    ...
    if (strcmp(path, "/") == 0 || strcmp(path, "/root") == 0) {
        terminal_printf("  [DIR]  system  (id=2)\n");
        terminal_printf("  [DIR]  users   (id=3)\n");
        terminal_printf("  [DIR]  files   (id=4)\n");
    } else if (strcmp(path, "/root/system") == 0) {
        terminal_printf("  [DIR]  kernel\n");
        terminal_printf("  [DIR]  drivers\n");
    } else if (strcmp(path, "/root/users") == 0) {
        terminal_printf("  [DIR]  default\n");
    } else if (strcmp(path, "/root/files") == 0) {
        terminal_printf("  [FILE] readme.txt (128 bytes)\n");
        terminal_printf("  [FILE] hello.bin  (1024 bytes)\n");
    }
    return 0;
}
...
int chrysfs_read_file(const char *path, void *buf, uint32_t max_size) {
    ...
    if (strcmp(path, "/root/files/readme.txt") == 0) {
        const char *msg = "Welcome to ChrysalisOS! This file is stored on the AHCI drive.\n";
        uint32_t len = strlen(msg);
        if (len > max_size) len = max_size;
        memcpy(buf, msg, len);
        return len;
    } else if (strcmp(path, "/root/files/hello.bin") == 0) {
        const char *msg = "Binary data here...";
        uint32_t len = strlen(msg);
        if (len > max_size) len = max_size;
        memcpy(buf, msg, len);
        return len;
