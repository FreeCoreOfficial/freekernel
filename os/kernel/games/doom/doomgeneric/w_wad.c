...
wad_file_t *W_AddFile (char *filename)
{
    ...
    else 
    {
        ...
        /* DEBUG: Verify WAD directory reading */
        {
            int i;
            for (i = 0; i < 10 && i < header.numlumps; i++) {
                serial("[WAD] lump[%d] pos=%d size=%d name='%.8s'\n",
                       i, fileinfo[i].filepos, fileinfo[i].size, fileinfo[i].name);
            }
        }
...
static void W_Name8(char out[8], const char *in)
{
    memset(out, 0, 8);
...
int W_CheckNumForName (char* name)
{
    ...
    else
    {
        ...
        {
            if (!memcmp(lumpinfo[i].name, name8, 8))
            {
                return i;
            }
        }
