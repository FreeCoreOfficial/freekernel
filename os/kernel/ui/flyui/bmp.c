...
extern int fat32_read_file_offset(const char *path, void *buf, uint32_t size,
                                  uint32_t offset);
...
int fly_load_bmp_to_surface(surface_t *surf, const char *path) {
  ...
  BITMAPINFOHEADER *infoHeader =
      (BITMAPINFOHEADER *)(header_buf + sizeof(BITMAPFILEHEADER));
  ...
  /* BMP stochează de obicei bottom-up. Rândul 0 din fișier este rândul de jos
   * al imaginii. */
  ...
  for (int i = 0; i < absHeight; i++) {
    ...
    /* Dacă height > 0 (bottom-up), rândul 0 din fișier este ultimul rând de pe
     * ecran */
    int screenY = (height > 0) ? (absHeight - 1 - i) : i;

    if (screenY >= (int)surf->height)
      continue;
    ...
    for (int x = 0; x < width; x++) {
      ...
      if (bpp == 24) {
        ...
        /* uint8_t a = px[3]; */
        color = 0xFF000000 | (r << 16) | (g << 8) | b;
        /* Notă: Multe BMP-uri 32bpp au alpha 0, deci forțăm FF la alpha pentru
         * vizibilitate */
      }
...
surface_t *fly_load_bmp(const char *path) {
  ...
  surface_t *s = surface_create(width, absHeight);
  if (!s)
    return NULL;

  if (fly_load_bmp_to_surface(s, path) != 0) {
    surface_destroy(s);
    return NULL;
  }
